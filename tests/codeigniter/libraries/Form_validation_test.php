...
class Form_validation_test extends CI_TestCase
{
    public function set_up()
    {
        ...
        $loader = $this->getMock('CI_Loader', ['helper']);

        // Same applies for lang
        $lang = $this->getMock('CI_Lang', ['load']);
    ...
    {
        $rules = [['field' => 'foo', 'label' => 'foo_label', 'rules' => 'required']];
        $this->assertTrue($this->run_rules($rules, ['foo' => 'bar']));
    ...
    public function test_rule_matches()
    {
        ...
        $this->assertTrue($this->run_rules($rules, array_merge($values_base, ['bar' => ''])));
    ...
    public function test_prep_for_form()
    {
        ...
        $this->form_validation->set_rules('foo', 'label', 'required', ['required' => $error_msg_unprepped]);
        $_POST = ['foo' => ''];
        $this->form_validation->run();
        $error_arr = $this->form_validation->error_array();

        $this->assertEquals('', $this->form_validation->prep_for_form(''));
        $this->assertEquals(['foo' => $error_msg_prepped], $this->form_validation->prep_for_form($error_arr));
    }
