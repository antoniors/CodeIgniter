...
class Loader_test extends CI_TestCase
{
    ...
    public function test_load_view()
    {
        ...
        // Mock output class
        $output = $this->getMock('CI_Output', ['append_output']);
        $output->expects($this->once())->method('append_output')->with($content.$value);
        $this->ci_instance_var('output', $output);

        // Test view output
        $this->assertInstanceOf('CI_Loader', $this->load->view($view, [$var => $value]));
    }
    ...
    public function test_initialize()
    {
        ...
        $path = APPPATH.$dir.'/';
        $model = 'Automod';
        $this->ci_vfs_create($model, '<?php class '.$model.' { }', $this->ci_app_root, [$dir, 'models']);
